...
import { type BookingStatus, getNextCancelStatus, getCancelToastMessage, getCancelErrorMessage } from '@/lib/travelmanager/booking-cancel-utils';
...
export default function BookingDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const handleCancelToggle = async () => {
    if (!booking) return;
    const nextStatus = getNextCancelStatus(booking.status);
    setCancelling(true);
    try {
      const res = await fetch(`/api/bookings/${id}`, {
        method: 'PUT',
      ...
      if (!res.ok) throw new Error();
      const updated: BookingData = await res.json();
      setBooking(updated);
      showToast(getCancelToastMessage(nextStatus));
    } catch {
      showToast(getCancelErrorMessage(nextStatus), 'error');
    } finally {
      setCancelling(false);
    }
  };
          ...
          <>
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className={`text-2xl font-bold ${booking.status === 'CANCELLED' ? 'text-slate-400 line-through' : 'text-slate-800'}`}>{booking.provider}</h1>
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  disabled={cancelling}
                  className={booking.status === 'CANCELLED' ? 'text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50' : 'text-amber-600 hover:text-amber-700 hover:bg-amber-50'}
                >
                  {cancelling ? (
