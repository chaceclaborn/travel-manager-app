...
export async function POST(request: NextRequest) {
  try {
    ...
    if (ids.length > MAX_BULK_IDS) {
      return NextResponse.json({ error: `Cannot delete more than ${MAX_BULK_IDS} vendors at once` }, { status: 400 });
    }
    ...
    const deleted = await prisma.$transaction(async (tx) => {
      const owned = await tx.vendor.findMany({
      ...
      const result = await tx.vendor.deleteMany({
    ...
    console.error('Error bulk-deleting vendors:', error instanceof Error ? error.message : error);
