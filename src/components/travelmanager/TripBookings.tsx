...
export function TripBookings({ tripId, tripStartDate, tripEndDate }: TripBookingsProps) {
  ...
  const handleCancelBooking = async (id: string, nextStatus: BookingStatus) => {
    try {
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {bookings.map((booking, i) => (
            <BookingCard key={booking.id} booking={booking} onDelete={setDeleteTarget} onEdit={startEdit} onCancel={handleCancelBooking} index={i} />
          ))}
        </div>
      )}
