...
function BookingCard({ booking, onDelete, onEdit, onCancelToggle, index }: { booking: Booking; onDelete: (id: string) => void; onEdit: (booking: Booking) => void; onCancelToggle: (id: string, nextStatus: BookingStatus) => Promise<void>; index: number }) {
  ...
  const handleCancelClick = async () => {
    ...
    try {
      await onCancelToggle(booking.id, isCancelled ? 'ACTIVE' : 'CANCELLED');
    } finally {
...
export function TripBookings({ tripId, tripStartDate, tripEndDate }: TripBookingsProps) {
  ...
  const handleCancelToggle = async (id: string, nextStatus: BookingStatus) => {
    try {
      ...
      {loading ? (
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {bookings.map((booking, i) => (
            <BookingCard key={booking.id} booking={booking} onDelete={setDeleteTarget} onEdit={startEdit} onCancelToggle={handleCancelToggle} index={i} />
          ))}
        </div>
      )}
