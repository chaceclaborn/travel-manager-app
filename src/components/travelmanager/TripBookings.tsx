...
function BookingCard({ booking, onDelete, onEdit, onCancel, index }: { booking: Booking; onDelete: (id: string) => void; onEdit: (booking: Booking) => void; onCancel: (id: string, nextStatus: 'ACTIVE' | 'CANCELLED') => Promise<void>; index: number }) {
  ...
  return (
    <motion.div
      ...
      initial="hidden"
      animate="visible"
      className={`rounded-lg border bg-white p-4 shadow-sm transition-all duration-200 ${isCancelled ? 'border-slate-200 bg-slate-50/60 opacity-70' : `border-slate-100 ${config.borderAccent} hover:-translate-y-0.5 hover:shadow-md`}`}
    >
      <div className="mb-3 flex items-start justify-between">
        <div className="flex items-center gap-2.5">
          ...
          <div>
            <p className={`font-semibold ${isCancelled ? 'line-through text-slate-400' : 'text-slate-800'}`}>{booking.provider}</p>
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            className={`cursor-pointer rounded-md p-2 transition-all duration-200 ${isCancelled ? 'text-slate-300 hover:bg-emerald-50 hover:text-emerald-500' : 'text-slate-300 hover:bg-amber-50 hover:text-amber-500'}`}
...
export function TripBookings({ tripId, tripStartDate, tripEndDate }: TripBookingsProps) {
  ...
  const handleCancel = async (bookingId: string, nextStatus: 'ACTIVE' | 'CANCELLED') => {
    try {
