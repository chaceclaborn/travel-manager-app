...
#### Auth refactor status (2026-04-06)

The server-side half of the Bearer refactor is **done**. `requireAuth()` now tries `Authorization: Bearer <token>` first and falls back to the existing cookie flow, so web traffic is unaffected and mobile clients can authenticate without cookies:

- `src/lib/travelmanager/auth.ts` — Bearer-first, cookie-fallback `requireAuth()`. An invalid bearer header returns 401 immediately rather than silently falling through to cookies.
- `src/lib/travelmanager/admin.ts` — unchanged; inherits Bearer support because it wraps `requireAuth()`.
- `src/lib/mobile-auth.ts` — new client helper. Exposes `isNativePlatform()`, `getStoredToken() / setStoredToken() / clearStoredToken()` (Keychain via `@capacitor/preferences` on native, `localStorage` fallback on web), and an `apiFetch()` wrapper that injects the Bearer header only on native runtimes.

`@capacitor/preferences` is loaded via dynamic import, so the package stays effectively optional until the Capacitor install step runs. Existing `fetch()` call sites in `src/` were **not** migrated — they keep working on web via cookies, and can move to `apiFetch` incrementally as features are touched for the mobile build.

#### Push notifications status (2026-04-06)

The scaffolding for push notifications is **in place**, but real APNs / FCM delivery is a follow-up task pending signing certificates from the Apple Developer portal.

What's wired up now:

- `public/sw.js` — service worker with offline-first cache, custom offline fallback, and a `push` + `notificationclick` handler. Skips `/api/*` so API calls still fail fast when offline (no stale-data UI).
- `public/offline.html` — minimal offline fallback page in the app's amber accent. Used by both the Web Push path and the WKWebView offline state.
- `src/components/travelmanager/ServiceWorkerRegister.tsx` — mounts in `src/app/(app)/layout.tsx`, registers `/sw.js` in production only (dev is skipped to avoid HMR clashes).
- `src/lib/push.ts` — `requestPushPermission()` and `registerPushListeners(onToken)`. Dynamic-imports `@capacitor/push-notifications` on native, falls back to the Web Notification API on browser. No-ops silently if the plugin isn't installed.
- `prisma/schema.prisma` — `DeviceToken` model (userId, token @unique, platform, timestamps) with a `User.deviceTokens` back-relation. `db push` + RLS applied.
- `prisma/enable-rls.sql` — `DeviceToken` table has RLS enabled (data only accessible through Prisma's postgres-role bypass, same pattern as every other table).
- `src/app/api/push/register/route.ts` — POST endpoint, auth'd + rate-limited + sanitized. Upserts a `DeviceToken` by token value so re-registrations are idempotent.
- `src/app/api/push/send/route.ts` — POST stub, auth'd + rate-limited + admin-gated. Currently logs the intended dispatch and returns the number of tokens that WOULD have been notified. Real delivery swap is a one-function change once the certs exist.

**Follow-up work to reach real delivery:**

1. Provision an APNs authentication key (`.p8`) in the Apple Developer portal under Certificates, Identifiers & Profiles → Keys.
2. Add the `AuthKey_XXXXXXXXXX.p8`, Key ID, and Team ID as environment variables on Vercel (server-only — never exposed to the client).
3. Replace the `console.log` stub in `src/app/api/push/send/route.ts` with either:
   - Direct APNs HTTP/2 calls (Node has `http2` built in — no package needed), OR
   - Firebase Cloud Messaging HTTP v1 (requires a service account JSON, also handles web push).
4. Wire `registerPushListeners` in the app entry point (post-login) so the token is POSTed to `/api/push/register` on first launch.
5. Trigger sends on domain events — booking confirmations, meeting reminders, trip status changes. These should call the send route with a system-level auth header (not the admin-email gate, which is only a placeholder).
6. Test end-to-end on a physical iPhone — the simulator does NOT deliver real pushes.
...
## 9a. Static export blockers (audit: 2026-04-06)

Generated by `yarn audit:static` (`scripts/audit-static-export.mjs`). Re-run any time. Blockers below will cause `next build` with `output: 'export'` to fail unless handled.

### Real blockers

_None._ The previously-blocking share page has been resolved (see below).

### Resolved blockers

| # | File | Issue | Resolution |
|---|---|---|---|
| 1 | `src/app/share/[token]/page.web.tsx` (was `page.tsx`) | `export const dynamic = 'force-dynamic'` + `async generateMetadata` that hits the DB via `getPublicTripByToken`. Next.js refuses to statically export a page marked `force-dynamic`. | **Resolved 2026-04-06 via `pageExtensions` + `.web.tsx` gating.** The file was renamed to `page.web.tsx` and `next.config.ts` now sets `pageExtensions` conditionally: web builds include `['web.tsx', 'web.ts', 'tsx', 'ts', 'jsx', 'js']` so the router picks it up at `/share/[token]`, while mobile builds use `['tsx', 'ts', 'jsx', 'js']` so `.web.tsx` files are invisible to the router and never enter the static export. This is intentional — clients open share links in a browser, not inside the iOS app, so the route doesn't need to exist in the mobile bundle. The audit script (`scripts/audit-static-export.mjs`) now partitions findings into pages / API / web-only and only counts pages toward the blocker total; the share page now appears under `[ok in .web.tsx files]` as informational. **To gate any future page out of mobile, name it `page.web.tsx`** and it'll be auto-excluded. |

### Potential blockers that are actually fine

The audit also flagged the following as `next/headers` imports, but all of them live in files only imported by API route handlers (which are not included in the static export bundle):

- `src/lib/supabase/server.ts` — imports `cookies` from `next/headers`. Only consumed by `src/app/auth/callback/route.ts` (route handler) and `src/lib/travelmanager/auth.ts`.
- `src/lib/travelmanager/auth.ts` — imports `headers` from `next/headers` and is consumed exclusively by `src/app/api/**/route.ts` files.

Route handlers are excluded from the `out/` directory by design ([Next.js docs](https://nextjs.org/docs/messages/api-routes-static-export)), so these imports never reach the bundled client. Keep an eye on this though — if a page component ever imports from `@/lib/travelmanager/auth` or `@/lib/supabase/server`, the mobile build will start failing.

### Middleware

`src/middleware.ts` **does not exist** — nothing to migrate. Good.

### Server Actions

`'use server'` directive search turned up **zero matches** across `src/`. The codebase is consistently client-fetch → API route, which is exactly the pattern that ports cleanly to Capacitor.

---

## 9b. Running the mobile build on a Mac (Next steps)

Capacitor was scaffolded in this repo on 2026-04-06. You can build the static web bundle from Windows (the dev machine) but creating the `ios/` Xcode project and running the app on a simulator/device **must** happen on a Mac with Xcode 26+.

### Deps added in the scaffold

Dependencies:
- `@capacitor/core@8.3.0` — runtime bridge
- `@capacitor/ios@8.3.0` — iOS platform
- `@capacitor/app@8.1.0` — app lifecycle + deep link handling
- `@capacitor/preferences@8.0.1` — Keychain-backed storage for session tokens
- `@capacitor/push-notifications@8.0.3` — APNs
- `@capacitor/share@8.0.1` — iOS share sheet
- `@capacitor/network@8.0.1` — network connectivity state for offline UX

Dev dependencies:
- `@capacitor/cli` — the `npx cap` tool

### One-time setup on the Mac

1. **Pull latest** — `git pull origin main`
2. **Install deps** — `yarn install` (picks up `@capacitor/*` packages added in this scaffold)
3. **Verify tooling** — `xcode-select --install`, confirm Xcode 26+ via `xcodebuild -version`
4. **Static-export blockers** — none currently (the share page blocker was resolved 2026-04-06; see section 9a). Re-run `yarn audit:static` before each mobile build to catch new ones.
5. **First mobile build** — `yarn build:mobile`. This produces `out/` (static HTML/JS).
6. **Create the iOS Xcode project** (one time only) — `yarn mobile:init`. This runs `npx cap add ios` and generates the `ios/` directory. It's in `.gitignore` so it stays on your Mac.
7. **Sync web assets into the iOS shell** — `yarn cap:sync`. Re-run this after every `yarn build:mobile`.
8. **Open Xcode** — `yarn cap:ios`. Xcode opens with the generated workspace.
...
```bash
yarn cap:sync      # rebuilds out/ and syncs to ios/App/App/public/
yarn cap:ios       # opens Xcode (or just hit Run in Xcode if it's already open)
```

### Xcode settings you'll need the first time

- **Signing & Capabilities → Team** — pick your Apple Developer Team
- **Signing & Capabilities → Bundle Identifier** — leave as `com.chaceclaborn.travelmanager` (matches `capacitor.config.ts`)
- **General → Deployment Info** — iOS 15.0 minimum (Capacitor 8 default)
- **General → Display Name** — "Travel Manager"
- **Run on simulator** — pick "iPhone 16 Pro" (or similar), hit ▶

### First TestFlight build (when you're ready)

1. Archive: Product → Archive
2. Distribute: Organizer window → Distribute App → App Store Connect → Upload
3. Apple processes the build (~10-30 min)
4. In App Store Connect → TestFlight, add the build to an internal test group
5. First external tester build must pass Beta App Review (~24-48h)
