...
|---|---|---|---|
| 1 | `src/app/share/[token]/page.tsx` | `export const dynamic = 'force-dynamic'` + `async generateMetadata` that hits the DB via `getPublicTripByToken`. Next.js refuses to statically export a page marked `force-dynamic`. | **Exclude from the mobile bundle.** The public share-link viewer is a web-only feature — travel agents don't use it inside the app. Simplest fix: move `src/app/share/` behind a conditional include, or set `pageExtensions` dynamically so `share` pages are only picked up on non-mobile builds. Needs ~30-60 min before the first successful `yarn build:mobile`. |
...
4. **Resolve the static-export blocker** above — `yarn build:mobile` will fail until `src/app/share/[token]/page.tsx` is gated out of the mobile build.
